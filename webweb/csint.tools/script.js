...
function changeTheme(theme) {
    ...
    Array.from(theme_list_items).forEach(item => {
        let a = item.firstChild; // Assuming the first child is an anchor element
        if (a) { // Check if 'a' exists
            ...
            console.warn('No anchor element found in item:', item);
        }
    });
    ...
    if (!document.head.contains(link) && !effectsDisabled) {
